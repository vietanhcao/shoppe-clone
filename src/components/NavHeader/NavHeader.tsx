...
export default function NavHeader() {
  ...
  const changeLanguage = (lng: keyof typeof locales) => {
        ...
        renderPopover={
          <div className='relative rounded-sm border border-gray-200 bg-white pr-32 text-sm shadow-md'>
            <div className='flex flex-col items-start px-3 py-1'>
              <button className='px-1 py-1 hover:text-orange' onClick={() => changeLanguage('vi')}>
                Tiếng Việt
              </button>
              <button className='mt-2 px-1 py-1 hover:text-orange' onClick={() => changeLanguage('en')}>
                English
              </button>
            </div>
          </div>
